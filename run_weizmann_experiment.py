...
DATA_ROOT = ROOT / "data" / "real_weizmann" / "partial" / "1obj"
...
def list_subset(limit: int = 12) -> list[str]:
    names = []
    for d in sorted(DATA_ROOT.iterdir()):
        if not d.is_dir():
            continue
        if list((d / "src_color").glob("*.png")) and list((d / "human_seg").glob("*.png")):
            names.append(d.name)
        if len(names) >= limit:
            break
    return names
...
def load_example(name: str, size: int) -> tuple[np.ndarray, np.ndarray]:
    color_path = next((DATA_ROOT / name / "src_color").glob("*.png"))
    image = Image.open(color_path).convert("RGB").resize((size, size), Image.BILINEAR)
    mask_paths = sorted((DATA_ROOT / name / "human_seg").glob("*.png"))
    masks = []
    for p in mask_paths:
        m = Image.open(p).convert("L").resize((size, size), Image.NEAREST)
        masks.append((np.asarray(m, dtype=np.uint8) > 200).astype(np.float64))
    gt = (np.mean(np.stack(masks, axis=0), axis=0) >= 0.5).astype(np.int64)
...
def main() -> None:
    ...
    for idx, name in enumerate(subset):
        ...
        for method in METHODS:
            model = run_method(method, x, CONFIG, seed=200 + idx)
    ...
    save_csv(
        OUT_DATA / "real_weizmann_per_image.csv",
        rows,
    ...
    save_csv(
        OUT_DATA / "real_weizmann_summary.csv",
        summary_rows,
    ...
    ax.set_ylim(0.0, 1.0)
    ax.set_ylabel("Score")
    ax.set_title("Weizmann 1-Object Subset: Real-Image Segmentation")
    ax.legend(frameon=False)
    ax.grid(axis="y", alpha=0.25, linestyle="--")
    fig.tight_layout()
    fig.savefig(OUT_FIG / "real_weizmann_summary.png", dpi=180)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(OUT_FIG / "real_weizmann_qualitative.png", dpi=180)
    plt.close(fig)

    with (OUT_DATA / "real_weizmann_config.txt").open("w", encoding="utf-8") as f:
        for key, value in CONFIG.items():
            f.write(f"{key}={value}\n")
        f.write(f"subset_size={len(subset)}\n")
        f.write("dataset=Weizmann 1-object partial subset from official archive\n")
